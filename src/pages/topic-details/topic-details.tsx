...
// Funcția pentru a obține rating din localStorage
...
const saveRatingsToLocalStorage = (commentId: number, rating: number) => {
  try {
    ...
    allRatings[commentId] = rating; // Actualizează ratingul pentru comentariul specific
    localStorage.setItem("commentRatings", JSON.stringify(allRatings)); // Salvează în localStorage
  } catch (error) {
...
const TopicDetails = () => {
  ...
  useEffect(() => {
    ...
    const sortedFetchedComments = fetchedComments.sort(
      (a, b) =>
        new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
    );
    ...
    const getReplies = async () => {
      ...
      const sortedReplies = commentsFromReplies.sort(
        (a: Comment, b: Comment) =>
          new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
      );

      // Actualizează ratingurile pentru comentariile obținute din replies
                      ...
                      .map((_, index) => (
                        <span
                          ...
                          className={`comment-star ${
                            comment.rating > index ? "selected" : ""
                          }`}
                          onClick={() =>
                            handleRatingClick(comment.id, index + 1)
                          }
                        >
                          ★
                        </span>
